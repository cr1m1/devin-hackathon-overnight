...
// Plan §9 / ADR-0003. The ONLY module that knows Devin v3 paths.
// Credentials are passed in per call (bring-your-own Devin: each Connection has its own key and org).
...
export type DevinCreds = { apiKey: string; orgId: string };

const apiBase = () => (process.env.DEVIN_API_BASE ?? "https://api.devin.ai").replace(/\/$/, "");
...
async function call<T>(creds: DevinCreds, path: string, opts: CallOpts = {}): Promise<T> {
  const { apiKey, orgId } = creds;
  const method = opts.method ?? "GET";
  const url = `${apiBase()}/${opts.versionPrefix ?? "v3"}/organizations/${orgId}${path}`;
...
export type DevinClient = ReturnType<typeof createDevinClient>;

export function createDevinClient(creds: DevinCreds) {
  return {
    async startSession(req: CreateSessionRequest): Promise<SessionResponse> {
      try {
        return await call<SessionResponse>(creds, "/sessions", { method: "POST", body: req, timeoutMs: 20_000, retries: 0 });
      } catch (e) {
    ...
    getSession(sessionId: string): Promise<SessionResponse> {
      return call<SessionResponse>(creds, `/sessions/${devinId(sessionId)}`, { retries: 2 });
    },
    async findSessionsByTag(tag: string): Promise<SessionResponse[]> {
      const q = new URLSearchParams({ tags: tag, first: "10" });
      const page = await call<Paginated<SessionResponse>>(creds, `/sessions?${q}`, { retries: 2 });
      return page.items;
    },
    messageSession(sessionId: string, message: string): Promise<unknown> {
      return call(creds, `/sessions/${devinId(sessionId)}/messages`, { method: "POST", body: { message }, retries: 2 });
    },
    archiveSession(sessionId: string): Promise<unknown> {
      return call(creds, `/sessions/${devinId(sessionId)}/archive`, { method: "POST", retries: 1 });
    },
    ...
    async listRepositories(): Promise<RepositoryInfo[]> {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const page: Paginated<RepositoryInfo> = await call<Paginated<RepositoryInfo>>(creds, `/repositories${q}`, { versionPrefix: "v3beta1", retries: 2 });
        out.push(...page.items);
...
/** Cheap credential check: one page of repositories. Throws DevinApiError(401/403) on a bad key or org. */
export async function verifyCreds(creds: DevinCreds): Promise<number> {
  const page = await call<Paginated<RepositoryInfo>>(creds, "/repositories", { versionPrefix: "v3beta1", retries: 1 });
